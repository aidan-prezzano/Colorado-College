...
def how_has_happiness_changed_past_15_years(df):
    ...
    # only lines concerning USA
    US = df[df["Country"] == country_1]

    US_happiness = US.groupby('Life Ladder')['year'].max()

    US_happiness_sorted = US_happiness.sort_values(ascending=False)

    US_happiness_sorted_last_15 = [US_happiness_sorted.head(15)]
    print(US_happiness_sorted_last_15)

    sns.lineplot(US_happiness_sorted_last_15)
    plt.show()
